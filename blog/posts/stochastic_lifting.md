...
Modern generative models (flow-based, diffusion-based) learn a multi-step transformation that transports a source distribution (usually noise) to a target data distribution. This can require often 100s of network evaluations in order to generate a new sample from the target distribution.
...
In this case entire multi-step process must be run *once per frame*, making long time generation computationally intractable. Even a modest 4-second video might require over 10,000 neural network evaluations, 

---

Our paper poses the follow question: **is the complexity of multi-step transport actually necessary for video generation?**
...
We are concerned with physics simulation and video generation. For both these problems we consider a stochastic setting, which leads to a particular set of challenges, 

1. **One-to-Many ambiguity**
  The same $\boldsymbol x_t$ can lead to *many* different realizations of $\boldsymbol x_{t+1}$.  This means *function* of the form  $F(\boldsymbol x_t) = \boldsymbol x_{t+1}$ can exist which exactly fits the data. Even if we consider two different but very close $\boldsymbol x_t$ and $\boldsymbol x_t'$, any reasonable smooth parametrization (such as a neural network) will not be able to fit the data.

2. **Mean collapse**
   Consider using a MSE loss such $||F(\boldsymbol{x_t})-\boldsymbol{x_{t+1}}||_2^2$ in the stochastic setting.

   This forces $F$ toward $\mathbb E[\mathbf X_{t+1}\mid\mathbf X_t=\boldsymbol x_t]$. That is we learn the mean dynamic, rather than the stochastic one.
...
3. Generate new samples by evaluating $F$ with fresh labels:
$$\boldsymbol{x_{t+1}'} = F(\boldsymbol{x_t'}, \boldsymbol{\xi_t'}) \quad\text{where}\quad\boldsymbol{\xi_t'} \sim \mathcal N(0, I_d)$$

**That's it**. The algorithm is very simple. In fact at first glance, its not clear why this would do anything meaningful.

## How does this help?

Lets consider why stochastic lifting would might avoid mean collapse. The method simply adds random inputs which are (by design) totally independent from the output. One might think because the labels contain no information about the output, the network should ignore them. But because each label is unique and fixed throughout training, the network can at least in theory interpolate the training data. This avoiding, at least on the training data, learning the conditional expectation.

But it is not obivious why interpolation on its own is helpful. We want to also ensure that we meaningfully generalize in a distributional sense such that when evulated on new labels $\boldsymbol{\xi_t'}$, our model $F(\boldsymbol{x_t'}, \boldsymbol{\xi_t'}) $ produces meaningful samples.

Our key insight is that if our function $F$ both **interpolates** and is **smooth**, then we can expect it to generate samples that are close to the true distribution. In paticular see proposition 1 in the paper:

$$WWW$$

See the full paper for the technical details. For now we can try to build an inuition for 

## Generalization: high-dimension and strong conditioning
